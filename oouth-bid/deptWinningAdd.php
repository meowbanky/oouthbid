...
ANY_VALUE(item_price.company_id) AS company_id,
ANY_VALUE(item_price.price) AS price,
ANY_VALUE(item_price.qty) AS qty,
ANY_VALUE(items.dept_id) AS dept_id,
ANY_VALUE(tbl_dept.dept) AS dept,
ANY_VALUE(tbl_company.company_name) AS company_name
FROM
tbl_evaluation
...
GROUP BY dept_id");
$dept = mysqli_query($bid,$query_dept) or die(mysqli_error($bid));
...
mysqlI_select_db($bid,$database_bid);
$query_itemSearch = sprintf("SELECT ANY_VALUE(item_price.price) AS price,ANY_VALUE(item_price.company_id) AS company_id, ANY_VALUE(items.dept_id) AS dept_id, ANY_VALUE(tbl_company.company_name) AS company_name, ANY_VALUE(items.item) AS item, ANY_VALUE(items.packSize) AS packSize, ANY_VALUE(items.qty) AS qty, ANY_VALUE(tbl_evaluation.percentage) AS percentage, ANY_VALUE(tbl_lot.lot_description) AS lot_description, ANY_VALUE(tbl_dept.dept) AS dept,
ANY_VALUE(item_price.item_id) AS item_id,ANY_VALUE(items.packSize) AS packSize,ANY_VALUE(items.qty) as require_year,
ANY_VALUE(items.qty*(tbl_evaluation.percentage/100)) as qtyAwarded FROM tbl_evaluation INNER JOIN item_price ON item_price.item_price_id = tbl_evaluation.item_price_id INNER JOIN tbl_company ON tbl_company.company_id = item_price.company_id
...
WHERE items.dept_id = %s GROUP BY item_price.company_id ORDER BY tbl_company.company_id", GetSQLValueString($col_itemSearch, "text"));
...
<div id="content-header" class="hidden-print sales_header_container">
	<h1 class="headigs"> <i class="icon fa fa-shopping-cart"></i>
		Print Winnings by Department &nbsp;<?php //echo $_SESSION['SESS_INVOICE'] ?><span id="ajax-loader"><img src="img/ajax-loader.gif" alt=""/></span>
			</h1>
		...
		<div class="sale_register_leftbox col-md-9">
			<div class="row forms-area">
				...
				<div class="col-md-4 no-padd">
			     ...
			     <form action="deptWinningAdd.php" method="post" accept-charset="utf-8" class="line_item_form" id="employee_form" autocomplete="off">
                     <label for="company">Select Department:
                  <select name="company" required="required" id="company" class="select_company">
                                 <option value="0">Select Department</option>
                                 <?php
do {
?>
                    <option value="<?php echo $row_dept['dept_id']?>"><?php echo $row_dept['dept']?></option>
                                 <?php
            ...
            <ul class="list-inline pull-left">
                <table id="register" class="table table-bordered">
                    ...
                    <?php if ($totalRows_itemSearch > 0) {
                        ...
                        do { ?>
                            <tr>
                                <td colspan="7"><strong><?php echo $row_itemSearch['company_name']; ?></strong></td>
                                <td align="right">&nbsp;</td>
                            </tr>
                            <?php
                            $query_companySpecific = sprintf("
                                    SELECT
                                        item_price.price,
                                        ...
                                        tbl_dept.dept,
                                        item_price.item_id,
                                        items.qty AS require_year,
                                        items.qty * (tbl_evaluation.percentage / 100) AS qtyAwarded
                                    FROM
                                        tbl_evaluation
                                    INNER JOIN item_price ON item_price.item_price_id = tbl_evaluation.item_price_id
                                    INNER JOIN tbl_company ON tbl_company.company_id = item_price.company_id
                                    ...
                                    INNER JOIN tbl_dept ON tbl_dept.dept_id = items.dept_id
                                    WHERE items.dept_id = %s AND item_price.company_id = %s
                                    ORDER BY tbl_company.company_name, items.item_id",
                                GetSQLValueString($row_itemSearch['dept_id'], "text"),
                                GetSQLValueString($row_itemSearch['company_id'], "text")
                            );
                            $companySpecific = mysqli_query($bid, $query_companySpecific) or die(mysqli_error($bid));
                            $row_companySpecific = mysqli_fetch_assoc($companySpecific);
                            $totalRows_companySpecific = mysqli_num_rows($companySpecific);
                            ...
                            if ($companySpecific) {
                                ...
                                do { ?>
                                    <tr>
                                        <td><?php echo $i; ?></td>
                                        <td><?php echo $row_companySpecific['item_id']; ?></td>
                                        <td><?php echo $row_companySpecific['item']; ?></td>
                                        <td width="65"><?php echo $row_companySpecific['packSize']; ?></td>
                                        <td width="78"><?php echo number_format($row_companySpecific['qtyAwarded']); ?></td>
                                        <td width="78"><?php echo $row_companySpecific['require_year']; ?></td>
                                        <td width="92"><?php echo number_format($row_companySpecific['price']); ?></td>
                                        <td width="92"><?php echo number_format($row_companySpecific['price'] * $row_companySpecific['qtyAwarded']); ?></td>
                                    </tr>
                                    <?php
                                    $i++;
                                    $subtotal += $row_companySpecific['price'] * $row_companySpecific['qtyAwarded'];
                                } while ($row_companySpecific = mysqli_fetch_assoc($companySpecific)); ?>
                                <tr>
                                    <td align="right" colspan="7"><strong>Subtotal:</strong></td>
                                    <td align="right"><strong><?php echo number_format($subtotal); ?></strong></td>
                                </tr>
                                <?php
                                $total += $subtotal;
                            }
                        } while ($row_itemSearch = mysqli_fetch_assoc($itemSearch)); ?>
                        <tr>
