...
function GetSQLValueString($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") 
{
  ...
  $Result1 = mysql_query($deleteSQL, $bid) or die(mysql_error());
...
$check = mysql_query($query_check, $bid) or die(mysql_error());
...
="utf-8" id="add_item_form" class="form-inline" autocomplete="off">								<input name="item" type="text" class="input-xlarge" id="item" placeholder="Enter item name or scan barcode" accesskey="i"
 value="" size="60"
  />																
				...
				<table id="register" class="table table-bordered">
					...
					<thead>
						<tr>
							...
							<th class="sales_quality">Price&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
							<th >Total</th>
						</tr>
					</thead>
					<tbody id="cart_contents" class="sa">
												<?php if ($totalRows_invoice > 0) {	?>	<?php do { ?>
													    <tr id="reg_item_top" bgcolor="#eeeeee" >
														    <td><a href="salesAdd.php?deleid=<?php echo $row_invoice['item_id'] ?>" class="delete_item"><i class="fa fa-trash-o fa fa-2x text-error"></i> </a></td>
														    ...
														    <td class="text text-warning sales_stock" id="reg_item_stock" ><span class="text text-info sales_item"><?php echo $row_invoice['lot_id']; ?></span></td>
														    ...
														    <td><span class="text text-info sales_item"><?php echo $row_invoice['packSize']; ?></span></td>
														    <td><span class="text text-info sales_item"><?php echo $row_invoice['lot_description']; ?></span></td>
														    <td><?php echo number_format($row_invoice['qty'],2); ?></td>
														    
													      <td width="10%"id="reg_item_qty">
												<form action="salesAdd.php" method="post" accept-charset="utf-8" class="line_item_form" autocomplete="off"><input name="price" type="text" required="required" class="input-small" id="price" accesskey="q" value="<?php echo number_format(ceil($row_invoice['price']),2); ?>" size="20" maxlength="8" />	<input name="sales_item_id" type="hidden" id="sales_item_id" value="<?php echo $row_invoice['item_id']; ?>" />												</form>
												</td>
														    
														    <td><span class="text text-main"><?php $total = ceil($row_invoice['price'] * $row_invoice['qty']); echo number_format($total,2) ?></span></td>
													    ...
													    <tr bgcolor="#eeeeee" >
													      ...
													      <td colspan="3" class="text text-info sales_item" id="reg_item_number3"><span class="line_item_form">
													        <form action="salesAdd.php" method="post" accept-charset="utf-8" class="line_item_form" autocomplete="off"><input name="remarks" type="text" class="input-small" id="remarks" accesskey="q" value="<?php echo $row_invoice['remarks']; ?>" size="50" />	<input name="item_id" type="hidden" id="item_id" value="<?php echo $row_invoice['item_id'] ?>" />												
												          </form>
													      </span></td>
													      <td>&nbsp;</td>
													      <td>&nbsp;</td>
							...
							<td colspan="11">
								<div class="text-center text-warning"> <h3></h3></div>
							</td>
						</tr>
										</tbody> <?php } ?>
					...
					<div id='sale_details'>
						<table id="sales_items" class="table">
							<tr class="warning">
								...
								<td class="right" align="right"><strong><?php echo number_format(($row_totalInvoice['total']*0.3),2) ;//$totalRows_invoice ; ?></strong></td>
							</tr>
														<tr class="success">
								<td align="right"><h3 class="sales_totals">Total:</h3></td>
								<td align="right"><h3 class="currency_totals"><?php echo number_format(ceil($row_totalInvoice['total']),2); ?></h3></td>
							</tr>
						</table>
					</div>
                    <li class="list-group-item">
				  <?php if ($totalRows_invoice > 0) {	?>
 <form action="print.php" method="post" accept-charset
="utf-8" id="finish_sale_form" autocomplete="off">						<input type='button' class='btn btn-success btn-large
 btn-block' id='finish_sale_button' value='Print' />					</div>
